...
- **Map lab (`map-lab.html`)** — drag a point across the complete singular curve in the local cuspidal-edge map `ℝ²→ℝ³`, then compare it directly with the paper's exact two-band projector map `T²→S²`. The four cusp points are marked as special points on the one-dimensional fold curve, not as the whole singular set.
